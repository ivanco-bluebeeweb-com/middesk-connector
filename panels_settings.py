...
(disconnect per Middesk account) for Middesk Connector. Split out of
panels.py per the same convention as DocuSign Connector's / CircleCI
...
def _connection_row(c: dict) -> ui.UINode:
    ...
    detail = f"{'Production' if env == 'production' else 'Sandbox'} · {h._mask(c.get('api_key', ''))}"
    return ui.Stack(direction="v", gap=1, align="start", children=[
        ui.Text(label, variant="body"),
        ui.Text(detail, variant="caption"),
        ui.Button(
            "Disconnect", variant="danger", size="sm",
            on_click=ui.Call("disconnect_middesk", {"connection_id": c.get("id")}),
        ),
    ])
...
def _connections_section(connections: list[dict]) -> ui.UINode:
    if not connections:
        return ui.Stack(direction="v", gap=1, children=[
            ...
            ui.Text("No Middesk accounts connected yet.", variant="caption"),
        ])
...
@ext.panel("middesk_settings", slot="center")
async def middesk_settings_panel(ctx) -> ui.UINode:
